...
def home(request):
	...
	if request.method == 'POST':
		df = pd.read_csv('C:\\Users\\DELL\\Desktop\\twideep\\gender_identification\\se.csv', encoding='latin1')
		df.head(10)
		'''positive_vocab = [ 'awesome', 'outstanding', 'fantastic', 'terrific', 'good', 'nice', 'great', ':)','hello','hey','hi' ]
		...
		df = pd.read_csv('C:/Users/DELL/Desktop/twideep/gender_identification/gender-classifier-DFE-791531.csv', encoding = 'latin1')
		...
		for tweet in all_tweets:
		    ...
		    if (str(tweet) == 'nan') or str(gender) == 'nan' or str(gender) == 'unknown' or float(gender_confidence) < 0.8:
		        c+=1
		        continue
		    ...
		    description_tweet_gender.append((tweet, gender))
		    c += 1
		...
		#description = "i like to sleep and i am professional dog lover."
		text = request.POST['text'].strip()
		features = find_features(top_words, text)
		result = NB_classifier.classify(features)
		print(result)
		...
		return render(request, 'result.html', {'result': result,'text': text, 'neg1': neg1, 'pos1': pos1, 'neu1': neu1})
	else:
		print("GET method")
